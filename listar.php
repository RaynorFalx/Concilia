...
<body>
 <div id="tabela_listar">
    ...
    <?php
        ...
        $query2 = "     SELECT Codigo, Processo, Vara, Parte, Horario, Resultado, Parte_Passiva
                        ...
                        if ($xml) {
                            ...
                            foreach($RetornoXML[1]->partes->parte as $partes){
                              $partes->tipo = strtoupper($partes->tipo);
                                ...
                                if (
                                $partes->tipo == "AGRAVANTE" || $partes->tipo == "REQUERENTE" || $partes->tipo == "APELANTE" || $partes->tipo == "RECORRENTE" || $partes->tipo == "IMPETRANTE" || 
                                $partes->tipo == "EMBARGANTE" || $partes->tipo == "INTERPELANTE" || $partes->tipo == "ASSISTENTE" || $partes->tipo == "AUTOR" || $partes->tipo == "RECTE" || 
                                $partes->tipo == "REMETENTE" || $partes->tipo == "PARTE AUTORA" || $partes->tipo == "OPOENTE" || $partes->tipo == "AMBARGANTE" || $partes->tipo == "AUTOR" ||
                    ...
                    if ($xml) {
                        ...
                        foreach($RetornoXML[1]->partes->parte as $partes){
                          $partes->tipo = strtoupper($partes->tipo); 
                            ...
                            if (
                                $partes->tipo == "AGRAVADO" || $partes->tipo == "REQUERIDO" || $partes->tipo == "APELADO" || $partes->tipo == "RECORRIDO" || $partes->tipo == "IMPETRADO" || 
                                $partes->tipo == "EMBARGADO" || $partes->tipo == "INTERPELADO" || $partes->tipo == "OPOENTE" || $partes->tipo == "RU" || $partes->tipo == "REU"  || 
                                $partes->tipo == "RECDO" || $partes->tipo == "PARTE RE" || $partes->tipo == "INDICIADO" || $partes->tipo == "PACIENTE" || $partes->tipo == "LITISCONSORTE PASSIVO" || 
                                $partes->tipo == "EMBARGADO" || $partes->tipo == "PROMOVIDO" || $partes->tipo == "IMPUGNADO" || $partes->tipo == "INVESTIGADO" || $partes->tipo == "QUERELADO" || 
                                $partes->tipo == "RECONVINDO" || $partes->tipo == "INVESTIGADO" || $partes->tipo == "DENUNCIADO A LIDE" || $partes->tipo == "ASSISTENTE PASIVO" || 
    ...
    //echo "</div>";
            ...
            while ($linha = oci_fetch_array($resultado2)) {
                ...
                if($parte !== NULL) {
                    ...
                    echo "<div class=\"td-print td-partes\">";
...
?>
    ...
    <br>
    <center>
        <button id="print" target="_blank" class="button2" name="print" onclick="printDiv2('printableArea')" value="Imprimir" style="padding:10px;margin:10px"> 
            Imprimir
        </button>
    </center>
    <?php
